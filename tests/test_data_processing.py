...
from src.data_processing import (
    DataProcessor, create_sample_dataset, create_classification_dataset, generate_time_series
)
...
class TestDataProcessor:
    ...
    def test_handle_missing_values_mean(self):
        result = self.processor.handle_missing_values(self.df, strategy='mean', columns=['numeric_col'])
    ...
    def test_handle_missing_values_median(self):
        result = self.processor.handle_missing_values(self.df, strategy='median', columns=['numeric_col'])
    ...
    def test_handle_missing_values_mode(self):
        result = self.processor.handle_missing_values(self.df, strategy='mode', columns=['categorical_col'])
    ...
    def test_encode_categorical_onehot(self):
        result = self.processor.encode_categorical(self.df, columns=['text_col'], method='onehot')
    ...
    def test_encode_categorical_label(self):
        result = self.processor.encode_categorical(self.df, columns=['text_col'], method='label')
    ...
    def test_scale_features_standard(self):
        ...
        result = self.processor.scale_features(df_numeric, columns=['col1', 'col2'], method='standard')
    ...
    def test_scale_features_minmax(self):
        ...
        result = self.processor.scale_features(df_numeric, columns=['col1'], method='minmax')
    ...
    def test_remove_outliers_iqr(self):
        ...
        result = self.processor.remove_outliers(df_with_outliers, columns=['values'], method='iqr')
    ...
    def test_remove_outliers_z_score(self):
        ...
        result = self.processor.remove_outliers(df_with_outliers, columns=['values'], 
    ...
    def test_invalid_columns(self):
        ...
        result = self.processor.handle_missing_values(self.df, columns=['nonexistent_col'])
...
class TestDatasetCreation:
    ...
    def test_create_classification_dataset(self):
        X, y = create_classification_dataset(n_samples=100, n_features=3, n_classes=4, random_state=42)
    ...
    def test_create_classification_dataset_reproducible(self):
        X1, y1 = create_classification_dataset(n_samples=50, n_classes=2, random_state=42)
        X2, y2 = create_classification_dataset(n_samples=50, n_classes=2, random_state=42)
    ...
    def test_generate_time_series_components(self):
        ...
        ts_trend = generate_time_series(n_points=100, trend=0.1, seasonality=0, noise=0, random_state=42)
        
        # Should be monotonically increasing
        assert np.all(np.diff(ts_trend) > 0)
        
        # Test with seasonality only
        ts_seasonal = generate_time_series(n_points=24, trend=0, seasonality=1, noise=0, random_state=42)
...
class TestDataProcessorIntegration:
    def test_full_preprocessing_pipeline(self):
        ...
        df = pd.DataFrame({
            ...
            'income': [50000, 60000, 55000, 80000, np.nan, 52000, 90000, 65000, 58000, 54000],
        ...
        df_encoded = processor.encode_categorical(df_filled, columns=['category', 'city'], method='onehot')
        
        # Check that categorical columns are encoded
        assert 'category' not in df_encoded.columns
        assert 'city' not in df_encoded.columns
        categorical_cols = [col for col in df_encoded.columns if col.startswith(('category_', 'city_'))]
        assert len(categorical_cols) > 0
        
        # Step 3: Scale numerical features
        numerical_cols = ['age', 'income']
        df_scaled = processor.scale_features(df_encoded, columns=numerical_cols, method='standard')
        
        # Check that numerical columns are scaled
        for col in numerical_cols:
            if col in df_scaled.columns:
                assert abs(df_scaled[col].mean()) < 1e-10
                assert abs(df_scaled[col].std() - 1.0) < 0.2  # More tolerant for small datasets
