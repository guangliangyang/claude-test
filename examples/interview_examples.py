...
def example_4_data_preprocessing():
    ...
    df_clean = processor.remove_outliers(
        df_filled, columns=["age", "income"], method="iqr"
    )
    print(f"\n移除异常值后数据量: {len(df)} -> {len(df_clean)}")

    # 3. 编码分类变量
    df_encoded = processor.encode_categorical(
        df_clean, columns=["education", "city"], method="onehot"
    )
    print(f"\n编码后特征数: {len(df_clean.columns)} -> {len(df_encoded.columns)}")
    print(
        f"新特征: {[col for col in df_encoded.columns if col.startswith(('education_', 'city_'))]}"
    )

    # 4. 标准化数值特征
    df_scaled = processor.scale_features(
        df_encoded, columns=["age", "income"], method="standard"
    )
