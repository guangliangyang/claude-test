...
class NaiveBayes(BaseEstimator, ClassifierMixin):
    ...
    def _gaussian_probability(self, x: float, mean: float, var: float) -> float:
        ...
        return (1 / np.sqrt(2 * np.pi * (var + epsilon))) * np.exp(-0.5 * ((x - mean) ** 2) / (var + epsilon))
...
def train_test_split(X: np.ndarray, y: np.ndarray, test_size: float = 0.2, random_state: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    if random_state:
